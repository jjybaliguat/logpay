...
export async function DELETE(req: Request) {
    ...
    if(!id) return NextResponse.json({message: "Missing required params"}, {status: 400})
    try {
        ...
        if(!employee) return NextResponse.json({message: `Employee with id ${id} not found.`}, {status: 404})
